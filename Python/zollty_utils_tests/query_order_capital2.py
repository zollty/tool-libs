...
bo_num AS (
    SELECT
        order_detail_id,
        part_no,
        SUM(IFNULL(bo_qty, 0)) - SUM(IFNULL(check_qty, 0)) AS total_bo
    FROM dms_parts.t_parts_sales_order_bo_detail
    WHERE order_no = %(orderNo)s
        AND del_flag = 0
    GROUP BY order_detail_id, part_no
...
pending_amount_summary AS (
    ...
    LEFT JOIN bo_num bn
        ON bn.order_detail_id = sod.id AND bn.part_no = sod.part_no
...
def main():
    ...
    # 输入 / 输出路径（直接改这里）
    # ============================================================
    input_file = r"D:\__SYNC2\git-dms\mexico-dms-web-vue3\order66.txt"
    output_file = "order_capital_result1.xlsx"
